...
def check_accounts_count(context, count):
    response = requests.get(base_url + "/api/accounts/count")
...
def delete_account(context, pesel):
    response = requests.delete(base_url + f"/api/accounts/{pesel}")
    assert_equal(response.status_code, 200)


@when("I save the account registry to db")
def save_accounts_to_db(context):
    response = requests.patch(base_url + "/api/accounts/save")
    assert_equal(response.status_code, 200)


@when("I load account registry from db")
def load_accounts_from_db(context):
    response = requests.patch(base_url + "/api/accounts/load")
    assert_equal(response.status_code, 200)
...
def update_last_name(context, pesel, last_name):
    response = requests.patch(
        base_url + f"/api/accounts/{pesel}", json={"last_name": last_name}
    )
    assert_equal(response.status_code, 200)
